...
class Partners extends BaseCommand {
    ...
    async addPartner(args){
        ...
        const successEmbed = this.client.createEmbed("{0} wurde als Partner hinzugefügt.", "success", "success", member.user.tag);
    ...
    async removePartner(args){
        ...
        if(!userdata.partner.state){
            const invalidOptionsEmbed = this.client.createEmbed("Dieses Mitglied ist kein Partner.", "error", "error");
            return this.message.reply({ embeds: [invalidOptionsEmbed] });
        }
        ...
        const successEmbed = this.client.createEmbed("{0} wurde als Partner entfernt.", "success", "success", member.user.tag);
        return this.message.reply({ embeds: [successEmbed] });
    }
