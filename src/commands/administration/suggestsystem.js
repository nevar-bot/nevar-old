...
class SuggestSystem extends BaseCommand {
    ...
    async enable(data){
        if(data.guild.settings.suggestions.enabled){
            const isAlreadyEnabled = this.client.generateEmbed("Das Ideen-System ist bereits aktiviert.", "error", "error");
        ...
        const successEmbed = this.client.generateEmbed("Das Ideen-System wurde aktiviert.", "success", "success");
        return this.interaction.followUp({ embeds: [successEmbed] });
    }

    async disable(data){
        if(!data.guild.settings.suggestions.enabled){
            const isAlreadyDisabled = this.client.generateEmbed("Das Ideen-System ist bereits deaktiviert.", "error", "error");
        ...
        const successEmbed = this.client.generateEmbed("Das Ideen-System wurde deaktiviert.", "success", "success");
        return this.interaction.followUp({ embeds: [successEmbed] });
    }

    async setChannel(channel, data){
        if(!channel){
            const invalidOptionsEmbed = this.client.generateEmbed("Du musst einen Channel angeben.", "error", "error");
        ...
        const successEmbed = this.client.generateEmbed("Neue Ideen werden absofort in {0} gesendet.", "success", "success", channel);
        return this.interaction.followUp({ embeds: [successEmbed] });
    }
    ...
    async setReviewChannel(channel, data){
        ...
        const successEmbed = this.client.generateEmbed("Neue Ideen werden absofort in {0} verwaltet.", "success", "success", channel);
        return this.interaction.followUp({ embeds: [successEmbed] });
    }
}
