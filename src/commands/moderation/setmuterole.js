...
class Setmuterole extends BaseCommand {
    ...
    async setMuteRole(role, data){
        ...
        if(role.id === this.interaction.guild.roles.everyone.id){
            const everyoneEmbed = this.client.generateEmbed("Die @everyone Rolle kann nicht als Mute-Rolle gesetzt werden.", "error", "error");
            return this.interaction.followUp({ embeds: [everyoneEmbed] });
        }

        // Role is managed by an integration
        if(role.managed){
            const roleIsManagedEmbed = this.client.generateEmbed("Rollen welche durch eine Integration verwaltet werden, können nicht als Mute-Rolle gesetzt werden.", "error", "error");
        ...
        if(this.interaction.guild.members.me.roles.highest.position <= role.position){
            const roleIsTooHighEmbed = this.client.generateEmbed("Da {0} eine höhere oder gleiche Position wie meine höchste Rolle ({1}) hat, kann sie nicht als Mute-Rolle gesetzt werden.", "error", "error", role, this.interaction.guild.members.me.roles.highest);
        ...
        if(data.guild.settings.muterole === role.id){
            const roleIsAlreadyMuteRoleEmbed = this.client.generateEmbed("{0} ist bereits die Mute-Rolle.", "error", "error", role);
        ...
        const successEmbed = this.client.generateEmbed("{0} wurde als Mute-Rolle gesetzt.", "success", "success", role);
