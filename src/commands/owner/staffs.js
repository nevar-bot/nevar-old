...
class Staffs extends BaseCommand {
    ...
    async addStaff(args){
        ...
        if(!args[1]){
            const invalidOptionsEmbed = this.client.generateEmbed("Du musst einen Staff-Typ angeben.", "error", "error");
            return this.message.reply({ embeds: [invalidOptionsEmbed] });
        }
        if(!['head-staff', 'staff'].includes(args[1].toLowerCase())){
            const invalidOptionsEmbed = this.client.generateEmbed("Du musst entweder \"staff\" oder \"head-staff\" als Staff-Typ angeben.", "error", "error");
            return this.message.reply({ embeds: [invalidOptionsEmbed] });
        }
        ...
        const successEmbed = this.client.generateEmbed("{0} wurde als {1} hinzugefügt.", "success", "success", member.user.tag, string);
    ...
    async removeStaff(args){
        ...
        if(!userdata.staff.state){
            const invalidOptionsEmbed = this.client.generateEmbed("Dieses Mitglied ist kein Staff.", "error", "error");
            return this.message.reply({ embeds: [invalidOptionsEmbed] });
        }
        ...
        const successEmbed = this.client.generateEmbed("{0} wurde als Staff entfernt.", "success", "success", member.user.tag);
        return this.message.reply({ embeds: [successEmbed] });
    }
